...
class AncestryDnaApp(tk.Frame):
    ...
    def __init__(self, master=None, gedcom_path: str = ""):
        ...
        self._matches_kit_guid_map  = self._state.matches_kit_guid_map
        self._matches               = self._state.matches
    ...
    def _build_main(self):
        ...
        # Matches-Tab (noch nicht extrahiert)
        frame = ttk.Frame(self._nb)
        self._tab_matches = frame
        self._nb.add(frame, text=self._t("tab_matches"))
        self._lang_nb_tabs.append((frame, "tab_matches"))
    ...
    def _refresh_match_table_inner(self, *_):
        ...
        if not all_sources_mode and not active_kit:
            active_kit = self._state.current_test_guid or self._get_kit_guid()
        ...
        if getattr(self, "_gedcom", None):
            try:
                tg = self._state.current_test_guid or self._get_kit_guid()
                if tg:
    ...
    def _on_match_select(self, _):
        ...
        test_guid_af = self._state.current_test_guid or self._get_kit_guid()
    ...
    def _on_close(self):
        ...
        if dl_running or (self._scraper and self._scraper.is_running()):
